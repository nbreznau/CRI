---
title: "Tech 002. Data Preparation"
output:
  html_document:
    df_print: paged
...
CRI Model Specifications and Margins in the file "CRI Model Specifications and Margins.xlsx". This model specification file was qualitatively coded by hand from each team's code and the margins came from `CRI_Expansion_All.Rmd` command file located in the folder **team_code\team_code_R**.

We set up two versions of the model specifications here: *cri* and *cri_str*. One is for dichotomous numerical values and the other is for string names of each variable (good for spec curve plotting). Participant survey also used here.
...
Recalibrate some variables. Respondents sometimes reported two conclusions because they concluded that they were conducting independent hypothesis tests (separte for 'flow' and 'stock' models). Adjust strings. 
...
```{r recodes1, warning=F, message=F}
...
```{r aspect_overview}
cri <- cri %>%
  mutate(belief_1_avg = mean(c(belief_H1_11,belief_H1_12,belief_H1_13), na.rm = T),
         belief_2_avg = mean(c(belief_H1_21,belief_H1_22,belief_H1_23), na.rm = T),
...
cri_r_aspects1 <- cri %>%
  subset(., u_teamid != 0) %>%
  ungroup() %>%
  summarise(belief_1 = round(mean(belief_1_avg, na.rm = T),1),
          belief_2 = round(mean(belief_2_avg, na.rm = T),1),
          pro_immigrant_mean = round(mean(pro_immigrant, na.rm = T),1),
          taught_stat_courses = round(mean(stat4, na.rm = T),1),
          mlm_familiarity = round(mean(stat5, na.rm = T),1),
          stat_pubs = round(mean(stat6, na.rm = T),1),
          topic_interest = round(mean(topic4, na.rm = T),1),
          topic_imm_pubs = round(mean(topic1, na.rm = T),1),
          topic_policy_pubs = round(mean(topic2, na.rm = T),1))

cri_r_aspects2 <- cri %>%
  subset(., u_teamid != 0) %>%
  ungroup() %>%
  summarise(belief_1_sd = round(sd(belief_1_avg, na.rm = T),1),
          belief_2_sd = round(sd(belief_2_avg, na.rm = T),1),
          pro_immigrant_sd = round(sd(pro_immigrant, na.rm = T),1),
          taught_stat_courses_sd = round(sd(stat4, na.rm = T),1),
          mlm_familiarity_sd = round(sd(stat5, na.rm = T),1),
          stat_pubs_sd = round(sd(stat6, na.rm = T),1),
          topic_interest_sd = round(sd(topic4, na.rm = T),1),
          topic_imm_pubs_sd = round(sd(topic1, na.rm = T),1),
          topic_policy_pubs_sd = round(sd(topic2, na.rm = T),1))
...
cri_r_aspects3 <- cri %>%
  ...
  summarise(belief_1_min = 1,
          belief_2_min = 1,
          pro_immigrant_min = min(pro_immigrant, na.rm = T),
          taught_stat_courses_min = min(stat4, na.rm = T),
          mlm_familiarity_min = min(stat5, na.rm = T),
          stat_pubs_min = min(stat6, na.rm = T),
          topic_interest_min = min(topic4, na.rm = T),
          topic_imm_pubs_min = min(topic1, na.rm = T),
          topic_policy_pubs_min = min(topic2, na.rm = T))
...
cri_r_aspects4 <- cri %>%
  ...
  summarise(belief_1_max = 5,
          belief_2_max = 5,
          pro_immigrant_max = max(pro_immigrant, na.rm = T),
          taught_stat_courses_max = max(stat4, na.rm = T),
          mlm_familiarity_max = max(stat5, na.rm = T),
          stat_pubs_max = max(stat6, na.rm = T),
          topic_interest_max = max(topic4, na.rm = T),
          topic_imm_pubs_max = max(topic1, na.rm = T),
          topic_policy_pubs_max = max(topic2, na.rm = T))
...
write.csv(cor1, file = here::here("results/TblS3_measrement.csv"))
...
write.csv(fit_fig, file = here::here("results/fit_measurement.csv"))
...
write.csv(cri_part, file = here::here("data","cri_indv.csv"), row.names = F)
...
png(filename = here::here("results","SEM.png"), width = 720, height = 620, res = 144)
sem_out
dev.off()
```
...
For the shiny app and some of our initial specification curve probing we used the package [rdfanalysis](https://joachim-gassen.github.io/rdfanalysis/) which requires discrete chocie variables. Therefore we code continuous researcher characteristics into categories here.
...
write.csv(cri, file = here::here("data","cri.csv"), row.names = F)
write.csv(cri_str, file = here::here("data","cri_str.csv"), row.names = F)
write.csv(cri_team, file = here::here("data","cri_team.csv"), row.names = F)
write.csv(cri_indiv, file = here::here("data","cri_indv.csv"), row.names = F)
...
write.csv(cri_team_combine, file = here::here("data","cri_team_combine.csv"), row.names = F)
...
In the folder **/CRI/CRI_shiny** we deploy a 'shiny app' to allow users to interact with the results by model specification types. 

```{r shiny, warning= F, message = F}
### Transform values from 1 to column names for all countries 

cri_shiny <- read.csv(here::here("data","cri.csv"), header = T)
cri_shiny <- cri_shiny %>%
  mutate_at(
    vars(AU:VE),
    funs(as.character(.))
...
# Team-level 
cri_shiny_team <- read.csv(file = here::here("data","cri_team.csv"), header = T)
...
cri_shiny_team <- cri_shiny_team %>%
  mutate(iv_type2 = ifelse(Stock == 1, "Stock", "Flow"),
         ...
         topic_ipredm = (topic_ipred - min(topic_ipred, na.rm = T))/(max(topic_ipred - min(topic_ipred, na.rm = T), na.rm = T)),
         belief_ipredm = (belief_ipred - min(belief_ipred, na.rm = T))/(max(belief_ipred - min(belief_ipred, na.rm = T), na.rm = T)),
         stats_ipredm = (stats_ipred - min(stats_ipred, na.rm = T))/(max(stats_ipred - min(stats_ipred, na.rm = T), na.rm = T)),
         pro_immigrantm = (pro_immigrant - min(pro_immigrant, na.rm = T))/(max(pro_immigrant - min(pro_immigrant, na.rm = T), na.rm = T)),
...
cri_shiny <- cri_shiny %>%
  subset(u_teamid != 0) %>%
  select(-c(additionalinfo, fbXnet:fbXleftright,backgr_degree_other1:belief_2_avg))
  
cri_shiny_team <- cri_shiny_team %>%
  subset(u_teamid != 0) %>%
  select(-c(fbXnet:fbXleftright,backgr_degree1:model_eval3))
...
write.csv(cri_shiny, file = here::here("shiny/data/cri_shiny.csv"), row.names = F)
write.csv(cri_shiny_team, file = here::here("shiny/data/cri_shiny_team.csv"), row.names = F)
