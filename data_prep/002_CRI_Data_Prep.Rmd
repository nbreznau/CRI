...
Recalibrate some variables. Respondents sometimes reported two conclusions because they concluded that they were conducting independent hypothesis tests (seperate for 'flow' and 'stock' models). Adjust strings. 
...
```{r recodes1, warning=FALSE, message=F}
...
```{r votes, message = F, warning=FALSE}
...
```{r team_data, message = F, warning=FALSE}
cri_team <- cri
...
```{r predict_sem, warning=FALSE, message=F}
# predicted factor scores
...
For the Shiny app and some of our initial specification curve probing we used the package [rdfanalysis](https://joachim-gassen.github.io/rdfanalysis/) which requires discrete choice variables. Therefore we code continuous researcher characteristics into categories here.
...
```{r save_out, warning=FALSE, message=F}
