...
cri_r_aspects1 <- cri %>%
  subset(., u_teamid != 0) %>%
  ungroup() %>%
  summarize(belief_1 = round(mean(belief_1_avg, na.rm = T),1),
...
cri_r_aspects2 <- cri %>%
  subset(., u_teamid != 0) %>%
  ungroup() %>%
  summarize(belief_1_sd = round(sd(belief_1_avg, na.rm = T),1),
...
write.csv(cri_r_aspects, file = "results/tbl_rcharacteristics_small.csv")
...
# Save shiny data
write.csv(cri_shiny, file = here::here("CRI_shiny","data","cri_shiny.csv"), row.names = F)
write.csv(cri_shiny_team, file = here::here("CRI_shiny","data","cri_shiny_team.csv"), row.names = F)
