...
pacman::p_load("readstata13","xlsx","dplyr","tidyr","ggplot2","kableExtra","GLMMadaptive","jtools","sjPlot","sjmisc","sjlabelled","margins","fastDummies","xlsx","ragg")
...
```{r load}
model_desc <- read.xlsx("data/Research Design Votes.xlsx", sheetName = "RD Paragraph Output", header = T, stringsAsFactors = F)
crispec <- read.xlsx("data/CRI Model Specifications and Margins.xlsx", sheetName = "Data")
votes <- read.dta13("data/votes.dta")
```

### Unipark Survey

The first file "Research Design Votes.xlsx" is the file we used to generate the model descriptions in the Unipark survey. 
The second file "CRI Model Spec...xlsx" is the main file with team model coding

The third file "votes.dta" is the average of participants' votes by research design presented in the survey. Note that these designs do not perfectly match the final research designs, therefore we cannot apply them directly to the models but must identify the ranking of models based on certain components. 
...
agg_png(file = "results/FigX_modelrank.png", width = 750)
...
dev.off()
knitr::include_graphics("results/FigX_modelrank.png")
```
...
save(popdf_out, file = "data/popdf_out.Rdata")
