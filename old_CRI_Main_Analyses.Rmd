...
df <- df %>%
  mutate(est = AME_Z,
         AMEssd = ifelse(iv_type == "Stock", AME_Z, NA),
         AMEfsd = ifelse(iv_type == "Flow", AME_Z, NA),
         AMEssd = sd(AMEssd, na.rm = T),
         AMEfsd = sd(AMEfsd, na.rm = T),
         estz = AME_Z*(AMEssd/AMEfsd),
         lb = lower_Z,
         ub = upper_Z,
         lbz = lower_Z*(AMEssd/AMEfsd),
         ubz = upper_Z*(AMEssd/AMEfsd),
         est = ifelse(iv_type == "Flow", estz, AME_Z),
         lb = ifelse(iv_type == "Flow", lb, lbz),
         ub = ifelse(iv_type == "Flow", ub, ubz),
...
df <- df %>%
  mutate(est = round(est, 5),
         lb = round(est, 5),
         ub = round(est, 5),
         lb = ifelse(est == lb, lb - 0.00001, lb),
         ub = ifelse(est == ub, ub + 0.00001, ub))
# remove missing
...
df2 <- as.data.frame(select(df, dv_type, Hresult, iv_type, software, est, lb, ub)) 
df2 <- completeFun(df2, c("est","lb","ub"))
# 
...
#agg_png(file = "data/Fig1_Overview.png", width = 800, res = 144,  bit = 16)
png(filename = "data/Fig1_Overview.png", width = 960, height = 720, res = 144)
...
knitr::include_graphics("data/Fig1_Overview.png")
...
png(filename = "data/Fig2_Popular.png", width = 960, height = 720, res = 144)
...
knitr::include_graphics("data/Fig2_Popular.png")
