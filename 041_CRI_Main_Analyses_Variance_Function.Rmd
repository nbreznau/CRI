...
cri_teams <- cri_teams %>%
    mutate(AME_team_var = ifelse(is.na(AME_team_var), 0, AME_team_var), # here NA means a single model or no variance, recode to 0
           AME_team_var2 = ifelse(AME_team_var > 0.03, 0.03, AME_team_var),  # three teams are insane outliers
...
# not used but potentially useful plots
...
cri_team_groups <- cri_teams %>%
  ...
  select(AME_team_int, AME_team_var, AME_team_int, beliefZ, proZ, statsZ, topicZ, belief_ipred, pro_immigrant, stats_ipred, topic_ipred)
...
```{r rainclouds_b}
g1 <-ggplot(cri_team_g, aes(x = group, y = AME_team_int)) +
...
```{r combine}
agg_png(filename = "results/Fig2_rain.png", width = 1000, height = 800, res = 144)
ggarrange(g1,g2)
dev.off()

knitr::include_graphics("results/Fig2_rain.png")
```
