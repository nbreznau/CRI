...
### Provided by the teams
       ...
       issp1985_2016.dta
       pooled.dta (created in Stata from CRI_Expansion_All.do)
       issp_l2.RData
       un_immigrants_emigrants_population.csv
...
#PI recode
data$net_migration <- data$net_migration/10
# Jobs
...
# models 13 to 16: Change in Immigrant Stock 
...
# models 17 to 20: Change in Immigrant Stock and Social Welfare Expenditures
...
# models 21 to 24: Change in Immigrant Stock and Employment rate
...
# models 25 to 28: Immigrant Stock with Interaction
...
# models 29 to 32: Change in Immigrant Stock with Interaction
...
pacman::p_load(tidyverse, plyr, haven, ordinal, miceadds, car, ggeffects)
...
datal2 <- datal2 %>%
  mutate(cntry = iso_country,                                        #just for commenting: 826 probably is GB not UK, might be wrong label
         ...
         ### PI Note, this value is the ratio of immigrants to natives multiplied by the popoulation divided by 1 million. This number results in very strange values that as far as we can tell have no logical interpretation. So we changed it to the actual stock measure that we think they intended to use
...
# PI rescale for comparability across teams
data$immgrStockCh <- data$immgrStockCh/10

#### Modeling the research questions ############################
### DV:`oldAge`###
oldAge_o_mod <- clm(oldAge_o ~ 1 + immgrStock*scale(immgrStockCh) + as.factor(cntry) + as.factor(year) + scale(gdp) + scale(gini), threshold = "flexible", data = data)
summary(oldAge_o_mod)

# PI note: the team does not reveal how they calculate average marginal effects. The margins command does not work for clm models so we go for ggemmeans marginal effects as the next best thing. Coefficients cannot be taken directly due to the interaction. Also, we have to average across the 4 categories of the DV.
m1 <- ggemmeans(oldAge_o_mod, terms = c("immgrStock [12.85,13.85]"), ci.lvl = 0.95)


m1a <- m1[1:4,]
m1b <- m1[5:8,]
m1c <- cbind(m1a,m1b)
m1c$delta <- m1c[9]-m1c[2]
m1c$error <- m1c[2]-m1c[4]
m1c <- m1c[,15:16]

team106 <- data.frame(matrix(NA, nrow = 8, ncol=4))
colnames(team106) = c("AME","lower","upper","id")
team106[1,1] = colMeans(m1c$delta)
### DV:`unemployed`###
unemployed_o_mod <- clm(unemployed_o ~ 1 + immgrStock*scale(immgrStockCh) + as.factor(cntry) + as.factor(year) + scale(gdp) + scale(gini), threshold = "flexible", data = data)
summary(unemployed_o_mod)



###PI note, there is a bug in the margins package (use traceback() to see) and it cannot handle the ols command, therefore we had to replicate the 'income' and 'jobs' models using lm() instead.

# Team's original ols code
...
incomeDiff_mod <- ols(scale(incomeDiff) ~ immgrStock + scale(immgrStockCh) + immgrStock*scale(immgrStockCh) + year + scale(gdp) + scale(gini),
                      x=T,
                      y=T,
                      data = data_ols,
                      na.action = na.delete)
...
###PI note, listwise deletion will be performed before the analysis to avoid new-level error in the margins phase.

data_ols_c <- data_ols[complete.cases(data_ols),]
data_ols_c$year <- factor(data_ols_c$year)

# PI adjusting SEs had to use lm and sandwich instead of ols and robcov
if (require("sandwich")) {
  incd <- lm(scale(incomeDiff) ~ immgrStock + scale(immgrStockCh) + immgrStock*scale(immgrStockCh) + year + scale(gdp) + scale(gini), data = data_ols_c)
  summary(margins(incd))
  ## heteroskedasticity-consistent covariance matrix
  incdame <- summary(margins(incd, vcov = vcovCL(incd, cluster = ~ cntry)))
}

incdame <- incdame[3:4,2:7]
incdame <- round(incdame,5)
...
### DV:`jobs`###
# the regression with immgrStock
jobs_mod <- ols(scale(jobs) ~ immgrStock + scale(immgrStockCh) + immgrStock*scale(immgrStockCh) + year + scale(gdp) + scale(gini),
                x=T,
                y=T,
                data = data_ols,
                na.action = "na.delete")
...
# PI adjusting SEs had to use lm and sandwich instead of ols and robcov
if (require("sandwich")) {
  jobs <- lm(scale(jobs) ~ immgrStock + scale(immgrStockCh) + immgrStock*scale(immgrStockCh) + year + scale(gdp) + scale(gini), data = data_ols_c)
  summary(margins(jobs))
  ## heteroskedasticity-consistent covariance matrix
  jobsame <- summary(margins(jobs, vcov = vcovCL(jobs, cluster = ~ cntry)))
}

jobsame <- jobsame[3:4,2:7]
jobsame <- round(jobsame,5)
...
teamrs <- c(3,12,16,18,20,28,35,40,41,48,56,59,61,62,69,70,77,82,84,87,93,94,98,104)
            #106
...
rout <- dplyr::bind_rows(team3,team12,team16,team18,team20,team28,team33,team35,team40,team41,team48,team56,team59,team61,team62,team69,team70,team77,team82,team84,team87,team93,team94,team98,team104)
...
#merge into Excel master file by hand for now
