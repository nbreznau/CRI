...
pacman::p_load(tidyverse, plyr, haven, ordinal, miceadds, car, ggeffects,rms)
...
## Marginal Effects
oldAge_o_mod_ME <- margins.oglmx(oldAge_o_mod)
oldAge_o_mod_ME
...
incomeDiff_mod_abs <- ols(scale(incomeDiff) ~ immgrStock_abs_m + scale(immgrStockCh) + immgrStock_abs_m*scale(immgrStockCh) + year + scale(gdp) + scale(gini),
                  x=T,
                  y=T,
                  data = data_ols,
                  na.action = "na.delete")
...
### Predictor immgrStock_abs_m
# the regression with immgrStock_abs_m
jobs_mod_abs <- ols(scale(jobs) ~ immgrStock_abs_m + scale(immgrStockCh) + immgrStock_abs_m*scale(immgrStockCh) + year + scale(gdp) + scale(gini),
                  x=T,
                  y=T,
                  data = data_ols,
                  na.action = "na.delete")
...
library(readxl)
read_xlsx("results.xlsx")%>%knitr::kable(., digits = 4)
results <- lapply(results, format, digits=4)

results[2:11] <- format(results[2:11], digits = 4)
results %>%
