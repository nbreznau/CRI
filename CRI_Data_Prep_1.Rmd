...
pacman::p_load("readstata13","dplyr","readr","ExPanDaR","lattice","tidyr","readxl","knitr","boot","ragg","kableExtra")
...
crispec <- crispec %>%
      ...
      mlm_str = ifelse(mlm_re_str == "Yes" | mlm_fe_str == "Yes" | hybrid_mlm_str == "Yes", "Yes", "No"),
...
crispec$dv_type <- car::recode(crispec$dv_type, 
                              "c('Scale_6', 'Scale_4', 'Scale_5', 'Scale_Desrv', 'Scale_Univ') = 'Scale';
                              'Jobs ' = 'Jobs'")

# Better labelling
crispec$indepv <- car::recode(crispec$indepv,
...
crispec$software <- car::recode(crispec$software,
                             "'stata' = 'Stata';
                             'r' = 'R';
                             'spss' = 'SPSS';
                             'mplus' = 'Mplus';
...
teamsize <- read.csv(file = wdir("teamsize.csv"), header = T)

cri <- left_join(cri, teamsize, by = "u_teamid")

rm(teamsize, cri2, crispec)
...
rm(cor,cor1,fit,popdf_out)

# save as csv to ensure no attributes carry over
write.csv(cri, file = wdir("Env1.csv"), row.names = F)
```
