...
m08vars <- " + twowayfe + w2016*twowayfe + mlm_fe + level_cyear + ml_glm + mlm_re + bayes + un_emp_rate_ivC + gdp_ivC + anynonlin"
...
```{r res_aspects_mlms}
m09 <- lmer(paste0(m04vars, m06vars, m08vars, "+ stats_ipred + topic_ipred + belief_ipred + (1 | team)"), data = cri_ml)
m10 <- lmer(paste0(m04vars, m06vars, m08vars, "+ stats_ipred + topic_ipred + belief_ipred + pro_immigrant + total_score + (1 | team)"), data = cri_ml) 
# Max variance
m11 <- lmer(paste0(m04vars, m06vars, m08vars, "+ stats_ipred + topic_ipred + belief_ipred + pro_immigrant + total_score + belief_ipred*topic_ipred	+ belief_ipred*stats_ipred + pro_immigrant*stats_ipred	+ topic_ipred*stats_ipred + (1 | team)"), data = cri_ml) 
...
tab_model(m00,m09,m10,m11, digits = 3, p.style = "stars", show.ci = F, file = "results/reg5.htm")

knitr::include_graphics("results/reg5.htm")

reg5 <- as.data.frame(read_html("results/reg5.htm") %>% html_table(fill=TRUE))
colnames(reg5) <- c("Effect","m00","m09","m10","m11")
...
reg5[44,2] <- m00v[1,4]
reg5[45,2] <- m00v[nrow(m00v),4]
reg5[44,3] <- m09v[1,4]
reg5[45,3] <- m09v[nrow(m09v),4]
reg5[44,4] <- m10v[1,4]
reg5[45,4] <- m10v[nrow(m10v),4]
reg5[44,4] <- m11v[1,4]
reg5[45,4] <- m11v[nrow(m11v),4]
```


```{r variance_explained_table}
tbl2 <- as.data.frame(matrix(nrow = 19, ncol = 4))

tbl2[1,2:4] <- c("Team-Levela (n=87)","Model-Levela (n=1,253)","Total")
tbl2[2:19,1] <- c("Baseline Variance","","Objective Conditions","Structure of Materials","Group Assignment","Subjective Decisions","Measurement","Data and Sample","Model Design","Researcher Attributes","Pro-Immigration Attitude","Knowledge of Topic","Statistical Skills","Peer Model Ranking", "Statistical Skills","","Unexplained Variance","aOf the 71 teams, 16 had independent tests and conclusions for stock and flow models leading to 87 team-level results. Of the 1,261 models submitted, 8 had convergence problems")

var_table <- matrix(nrow = 11, ncol = 3)
var_table[1,] <- c("m00",reg1[12,2],reg1[13,2])
var_table[2,] <- c("m01",reg1[12,3],reg1[13,3])
var_table[3,] <- c("m02",reg1[12,4],reg1[13,4])
```
