...
reg1[9,2] <- m00v[1,4]
reg1[10,2] <- m00v[nrow(m00v),4]
reg1[9,3] <- m01v[1,4]
reg1[10,3] <- m01v[nrow(m01v),4]
reg1[9,4] <- m02v[1,4]
reg1[10,4] <- m02v[nrow(m02v),4]
...
reg2[14,2] <- m00v[1,4]
reg2[15,2] <- m00v[nrow(m00v),4]
reg2[14,3] <- m03v[1,4]
reg2[15,3] <- m03v[nrow(m03v),4]
reg2[14,4] <- m04v[1,4]
reg2[15,4] <- m04v[nrow(m04v),4]
...
reg3[18,2] <- m00v[1,4]
reg3[19,2] <- m00v[nrow(m00v),4]
reg3[18,3] <- m05v[1,4]
reg3[19,3] <- m05v[nrow(m05v),4]
reg3[18,4] <- m06v[1,4]
reg3[19,4] <- m06v[nrow(m06v),4]
...
reg4[24,2] <- m00v[1,4]
reg4[25,2] <- m00v[nrow(m00v),4]
reg4[24,3] <- m07v[1,4]
reg4[25,3] <- m07v[nrow(m07v),4]
reg4[24,4] <- m08v[1,4]
reg4[25,4] <- m08v[nrow(m08v),4]
...
tab_model(m00,m09,m10,m11,m12,m13,m14, digits = 3, p.style = "stars", show.ci = F, file = "results/reg5.htm")

knitr::include_graphics("results/reg5.htm")

reg5 <- as.data.frame(read_html("results/reg5.htm") %>% html_table(fill=TRUE))
colnames(reg5) <- c("Effect","m00","m09","m10","m11","m12","m13","m14","m15")
...
reg5[25,2] <- m00v[1,4]
reg5[26,2] <- m00v[nrow(m00v),4]
reg5[25,3] <- m09v[1,4]
reg5[26,3] <- m09v[nrow(m09v),4]
reg5[25,4] <- m10v[1,4]
reg5[26,4] <- m10v[nrow(m10v),4]
reg5[25,5] <- m11v[1,4]
reg5[26,5] <- m11v[nrow(m11v),4]
reg5[25,6] <- m12v[1,4]
reg5[26,6] <- m12v[nrow(m12v),4]
reg5[25,7] <- m13v[1,4]
reg5[26,7] <- m13v[nrow(m13v),4]
reg5[25,8] <- m14v[1,4]
reg5[26,8] <- m14v[nrow(m14v),4]
#reg5[25,9] <- m15v[1,4]
#reg5[26,9] <- m15v[nrow(m15v),4]
```
...
var_table <- as.data.frame(matrix(nrow = 17, ncol = 3))
colnames(var_table) <- c("model","team_level","model_level")

# These are not data scientist ready, they need to be hand re-coded incase models above change!!!
var_table[2,] <- c("m00",reg1[9,2],reg1[10,2])
var_table[3,] <- c("m01",reg1[9,3],reg1[10,3])
var_table[3,] <- c("m02",reg1[9,4],reg1[10,4])

var_table[5,] <- c("m03",reg2[14,3],reg2[15,3])
var_table[6,] <- c("m04",reg2[14,4],reg2[15,4])
var_table[6,] <- c("m05",reg3[18,3],reg3[19,3])

var_table[8,] <- c("m06",reg3[18,3],reg3[19,4])
var_table[9,] <- c("m07",reg4[24,3],reg4[25,3])
var_table[10,] <- c("m08",reg4[24,4],reg4[25,4])

var_table[11,] <- c("m09",reg5[25,3],reg5[26,3])
var_table[12,] <- c("m10",reg5[25,4],reg5[26,4])
var_table[13,] <- c("m11",reg5[25,5],reg5[26,5])

var_table[14,] <- c("m12",reg5[25,6],reg5[26,6])
var_table[15,] <- c("m13",reg5[25,7],reg5[26,7])
var_table[16,] <- c("m14",reg5[25,8],reg5[26,8])
# var_table[16,] <- c("m15",reg5[25,9],reg5[26,9])



##### Stopped here, need to fit the labels into the table





var_table[1,2:4] <- c("Team-Levela (n=87)","Model-Levela (n=1,253)","Total")
var_table[2:19,1] <- c("Baseline Variance","","Objective Conditions","Structure of Materials","Group Assignment","Subjective Decisions","Measurement","Data and Sample","Model Design","Researcher Attributes","Pro-Immigration Attitude","Knowledge of Topic","Statistical Skills","Peer Model Ranking", "Statistical Skills","","Unexplained Variance","aOf the 71 teams, 16 had independent tests and conclusions for stock and flow models leading to 87 team-level results. Of the 1,261 models submitted, 8 had convergence problems")
...
var_table$team_var_reduc <- (var_table[1,2] - var_table$team_level)/var_table[1,2]
var_table$model_var_reduc <- (var_table[1,3] - var_table$model_level)/var_table[1,3]
var_table$total_var_reduc <- ((var_table[1,2] + var_table[1,3]) - (var_table$total)) / (var_table[1,2] + var_table[1,3])
