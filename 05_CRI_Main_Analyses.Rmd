...
pacman::p_load("ggplot2","dplyr","readr","plotscale","lattice","tidyr","readxl","mlogit","jtools","sjPlot","sjmisc","sjlabelled","knitr","kableExtra","lavaan","reshape2","semPlot","lavaanPlot")
...
# correlatinos
cor1 <- select(sem, -c(u_teamid,Group.1))
cor1 <- round(cor(cor1, use = "pairwise.complete.obs"),2)

sem_f <- completeFun(sem, c("Hsup","belief1"))
...
### Table X. Correlation Matrix of Researcher Backgrouds, Team-Level
...
```{r sem_fit}
fit1 <- summary(fit, fit.measures=TRUE, standardized = T)
...
sem_out <- lavaanPlot(model = fit, labels = labels, coefs = TRUE, covs = TRUE, stand = TRUE)

png(filename = "results/SEM.png", width = 720, height = 620, res = 144)
sem_out
dev.off()
```
This chunk essentially proves that it probably doesn't matter whether we take team-level measures or individual-level measures agggregated by team as correlations are very high (0.86-0.95). 

```{r predict_sem}
# predicted factor scores
pred <- as.data.frame(lavPredict(fit))
...
# correlatinos
cor2 <- select(cri_team_test, -c(u_teamid))
cor2 <- round(cor(cor2, use = "pairwise.complete.obs"),2)

rm(cor2,cri_team,cri_team_test,cri_team_test1,cri_team_test2)
```
...
### Model Specifications - Test IV Type - unweighted
...
### Model Specifications - Test IV Type - weighted
...
m1aw <- lm(AME_Z ~ Jobs + Unemp + IncDiff + OldAge + House + Health + Scale, data = cri, weight = inv_weight)
m1aw_stock <- lm(AME_Z ~ Jobs + Unemp + IncDiff + OldAge + House + Health + Scale, data = cri_stock, weights = inv_weight)
m1aw_flow <- lm(AME_Z ~ Jobs + Unemp + IncDiff + OldAge + House + Health + Scale, data = cri_flow, weights = inv_weight)
...
### Model Specifications - Sample Selection Related IVs
...
```{r m2}
m2 <- lm(AME_Z ~ orig13 + orig17 + eeurope + allavailable + w2016, data = cri)
m2_stock <- lm(AME_Z ~ orig13 + orig17 + eeurope + allavailable + w2016, data = cri_stock)
m2_flow <- lm(AME_Z ~ orig13 + orig17 + eeurope + allavailable + w2016, data = cri_flow)

tab_model(m2, m2_stock, m2_flow, p.style = "stars", show.ci = F, rm.terms = c("(Intercept)"))
