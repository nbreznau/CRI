...
server <- function(input, output, session) {
  ...
  output$p_val <- renderPlot(res = 200{
    if (nrow(dfspec2()) != 0){
      p1 <- ggplot(dfspec2()) +
        geom_errorbar(aes(x = count_new, ymin = p_lb, ymax = p_ub), color = "grey90")+
        geom_point(aes(x = count_new, y = p_new, color = p_sig_group), shape = "|", size = 3) +
      ...
      p2 <- 
        ggplot(dfspec2()) +
        geom_tile(aes(x = count_new, y = 0.4, fill = factor(Hsup), height = 0.133, width = 0.25)) +
        geom_tile(aes(x = count_new, y = 0.2, fill = factor(Hrej), height = 0.133, width = 0.25)) +
        scale_y_continuous(breaks = c(0.4,0.2), labels = c("Support\n   ", "Reject")) +
        scale_fill_manual(values = c("white","#66A61E","#D95F02")) +
        ylab("Team\nConclusion") +
        theme_classic() +
      ...
      p3 <- 
        ggplot(dfspec2()) +
        geom_tile(aes(x = count_new, y = 0.6, fill = factor(Stock)), height = 0.133, width = 0.25) +
        geom_tile(aes(x = count_new, y = 0.4, fill = factor(Flow)), height = 0.133, width = 0.25) +
        geom_tile(aes(x = count_new, y = 0.2, fill = factor(ChangeFlow)), height = 0.133, width = 0.25) +
        scale_y_continuous(breaks = c(0.6,0.4,0.2), labels=p3labs) +
        scale_fill_manual(values = c("white","blue")) +
        theme_classic() +
        ylab("Immigration\nMeasurement") +
