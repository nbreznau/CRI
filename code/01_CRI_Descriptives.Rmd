...
pacman::p_load("tidyverse","knitr","kableExtra","reshape","ggplot2","summarytools", "Hmisc", "ragg")
...
Note that Team 0 is actually the original study and hypothesis test on which these teams based their own work (Brady and Finnigan [2014](https://journals.sagepub.com/doi/full/10.1177/0003122413513022)). This is of potential interest for comparison, but removed in the final analyses.

```{r load, warning=FALSE,message=FALSE, include = FALSE}
cri <- read.csv(file = here::here("data/cri.csv"), header = TRUE) %>%
    subset(u_teamid != 0) # remove original study from results
...
Our sample is `r length(unique(cri_str$u_teamid))` teams including `r sum(cri_count$team_size)` researchers that submitted `r length(cri_str$u_teamid)` models. Of those teams, one never carried out the hypothesis test based on a failed measurement invariance test, another had all their `r length(cri_str$u_teamid[cri_str$u_teamid == 1])` models fail to converge and one other had one model out of several fail to converge. The result was `r length(unique(cri$u_teamid[!is.na(cri$AME)]))` teams with numeric results of their hypothesis tests derived from `r length(cri_str$u_teamid[!is.na(cri_str$AME)])` models produced by `r sum(cri_count$team_size)` researchers in total with an average of `r round(sum(cri_str$team_size)/length(cri_str$id),3)` researchers per team.
There were `r sum(cri_count$team_size)` participants in `r length(unique(cri$u_teamid))` teams that produced `r length(cri$u_teamid)` models, average of `r round(length(cri$u_teamid)/length(unique(cri$u_teamid)),3)` models per team.

There were `r sum(duplicated(cri_team$u_teamid))` teams that considered their two independent variables (stock and flow of immigration) as two independent hypothesis tests and therefore submitted two subjective conclusions. Therefore, we have `r length(cri_team$u_teamid)` observations ('results') among the team-level data that derive from `r length(unique(cri_team$u_teamid[!is.na(cri_team$AME)]))` teams. With an average of `r round(length(cri$u_teamid)/(length(unique(cri$u_teamid))+(sum(duplicated(cri_team$u_teamid))/2)),3)` models per team-conclusion unit of observation.

### Summary Stats

We no longer use this approach but leave these tables here in case of interest. The [codebook](../results/Tbl_codebook_desc.xlsx) is now our main summary statistics location. Reproduced here.

```{r codebookimage, message = F, warning = F}
knitr::include_graphics(here::here("results", "Tbl_codebook_desc.png"), dpi = 50)
```
...
knitr::include_graphics(here::here("results/Tbl_codebook.png"), dpi = 50)
```
...
knitr::include_graphics(here::here("results/Tbl_codebook2.png"), dpi = 50)
```
