...
The datasets in here were worked up starting in the sub-folder 'team_code' in the 'team_code_R.Rmd' file and then in the project sub-folder 'CRI/data_prep' 
...
# team stats
cri_count <- aggregate(cri_team, by = list(cri_team$u_teamid), FUN = "mean", na.rm = T)
```
...
```{r codebook, include = F, warning = F}
codebook <- subset(cri_str, u_teamid != 0)
codebook$DV[grepl("Scale",codebook$DV)] <- "Scale"
codebook <- select(codebook, DV, main_IV_type, main_IV_measurement, main_IV_time, num_countries, main_IV_as_control, software, mator, indepv, inv_weight) %>%
    mutate(num_countries = factor(num_countries),
           main_IV_as_control = factor(main_IV_as_control))

# summarytools::freq(codebook, order = "freq", style = "grid")

# Get weighted proportions
codebook1 <- gather(codebook, "var", "value", DV:indepv) %>%
  count(var, value, wt = inv_weight) %>%
  group_by(var) %>%    
  mutate(propw = prop.table(n)) %>%
  ungroup() %>%
  select(., propw)

# Get unweighted proportions plus n cases
codebook2 <- gather(codebook, "var", "value", DV:indepv) %>%
  count(var, value) %>%
  group_by(var) %>%             
...
#reorder decreasing
codebook1 <- codebook1[order(codebook1$var, codebook1$n, decreasing = T),]

# add labels
codebook1 <- codebook1 %>%
    mutate(value = ifelse(n == 1000, var, ifelse(n == 0, "total", value)))

codebook1 <- select(codebook1, -var)
...
kable_styling(t1) %>%
  as_image(width = 4, file = here::here("results","Table1.png"))
```

```{r table1, warning=F, message=F, echo=T}
knitr::include_graphics(here::here("results","Table1.png"))
```
...
kable_styling(t2) %>%
  as_image(width = 4, file = here::here("results","Table2.png"))

```

```{r table2, warning=F, message=F, echo=T}
knitr::include_graphics(here::here("results/Table2.png"))
```
...
agg_png(filename = here::here("results/Tbl1_cor.png"), res = 144, height = 600, width = 700)
...
knitr::include_graphics(here::here("results","Tbl1_cor.png"))
...
w_mean <- round(weighted.mean(cri$AME_Z, w = cri$inv_weight, na.rm = T),4)
w_var <- round(wtd.var(cri$AME_Z, weights = cri$inv_weight, na.rm = T),4)
```


```{r hist_results_basic}
g1 <-  ggplot(data = cri0) +
  #geom_density(aes(x = AME)) +
...
agg_png(filename = "results/FigS5.png", res = 144, width = 1000)
