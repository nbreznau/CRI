...
crimrg <- select(crimrg, degree, id)
...
ggplot(crispectest) +
  ...
  #ylim(-1.8, 1.8) +
  annotate(geom = "text", x = 150, y = 0.5, label = "NEGATIVE AME, 95%CI", color = "#66A61E", fontface = "bold", hjust = 0, size = 3) +
  annotate(geom = "text", x = 800, y = 1, label = "POSITIVE AME, 95%CI", color = "#D95F02", fontface = "bold", hjust = 0, size = 3) +
  annotate(geom = "text", x = 450, y = -0.6, label = "NOT SIGNIFICANT AME, 95%CI", fontface = 3, color = "grey55", hjust = 0, size = 2.5) +
  ...
  labs(color = "Effect at 95%CI", x = "Model Number, ordered by point estimate", y = "Average Marginal Effect (AME)") +
  scale_y_continuous(labels = c("-0.50","-0.05","0.00","0.05","0.50"), breaks = c(-1.75, -1, 0, 1, 1.75), limits = c(-1.8,1.8)) +
...
agg_png(filename = here::here("results/Fig1_new.png"), res = 144, height = 900, width = 1700)
...
```{r fig1_col1}
agg_png(filename = here::here("results/Fig1_Col1.png"), height = 800, width = 800, res = 72)
ggarrange(g1a, g2a, g01, g01ci, g02, g02ci, g03, g03ci, g04, g04ci, nrow = 5, ncol = 2, heights = c(0.6,.75,0.5,1.75,1.2), widths = c(1,0.9))
dev.off()

knitr::include_graphics(here::here("results/Fig1_Col1.png"))
```
...
ggplot(crispectest) +
 ...
 g9ci <-
ggplot(crispectest) +
  geom_tile(aes(x = count2, y = 4, fill = factor(W_96_06)), height = 0.5) +
  geom_tile(aes(x = count, y = 3, fill = factor(W_96_06_16)), height = 0.5) +
  geom_tile(aes(x = count2, y = 2, fill = factor(W_06_16)), height = 0.5) +
  geom_tile(aes(x = count2, y = 1, fill = factor(W_Other)), height = 0.5) +
  scale_y_continuous(breaks = c(4,3,2,1), labels=samplabs) +
  scale_fill_manual(values = c("NA","blue")) +
    theme_classic() +
  ylab("Survey Waves in Sample") +
...
g10ci <-
ggplot(crispectest) +
  geom_tile(aes(x = count2, y = 4, fill = factor(C_13)), height = 0.5) +
  geom_tile(aes(x = count2, y = 3, fill = factor(C_CEE)), height = 0.5) +
  geom_tile(aes(x = count2, y = 2, fill = factor(C_All)), height = 0.5) +
  geom_tile(aes(x = count2, y = 1, fill = factor(C_Other)), height = 0.5) +
  scale_y_continuous(breaks = c(4,3,2,1), labels=clabs) +
  scale_fill_manual(values = c("NA","blue")) +
    theme_classic() +
  ylab("Countries in Sample") +
...
g11ci <-
ggplot(crispectest) +
  geom_tile(aes(x = count2, y = 5, fill = factor(Est_ols)), height = 0.5) +
  geom_tile(aes(x = count2, y = 4, fill = factor(Est_logit)), height = 0.5) +
  geom_tile(aes(x = count2, y = 3, fill = factor(Est_mlglm)), height = 0.5) +
  geom_tile(aes(x = count2, y = 2, fill = factor(Est_ologit)), height = 0.5) +
  geom_tile(aes(x = count2, y = 1, fill = factor(Est_bayes)), height = 0.5) +
  scale_y_continuous(breaks = c(5,4,3,2,1), labels=estlabs) +
  scale_fill_manual(values = c("NA","blue")) +
    theme_classic() +
  ylab("Estimation Method") +
...
```{r fig1_col34}
agg_png(filename = here::here("results/Fig1_Col34.png"), height = 800, width = 800, res = 72)
ggarrange(g1a, g2a, g9, g9ci, g10, g10ci, g11, g11ci, nrow = 4, ncol = 2, heights = c(0.6,1,1,1.2), widths = c(1,0.9))
dev.off()

knitr::include_graphics(here::here("results/Fig1_Col34.png"))
```
...
g5a_new <- 
  ggplot(cri_team) +
  geom_point(aes(x = count2, y = AME_Zt, color = subj_concl), shape = 18, size = 4) +   scale_color_manual(values = c("firebrick","grey55", "chartreuse4"), labels = c("Rejected","Not testable","Supported")) +
...
agg_png(filename = here::here("results/Fig2team_new2.png"), res = 144, height = 900, width = 1200)
ggarrange(g5a_new,g5b_new,ncol = 1, heights = c(1,1.2))
dev.off()

knitr::include_graphics(here::here("results/Fig2team_new2.png"))
```
...
This routine takes long so we've commented it out and saved the results for re-loading below.

```{r clus_cut, warning = F, message = F, echo = F}
# k-means clustering
cri_km2 <- eclust(cri_spec_only2, "kmeans", nstart = 25)

saveRDS(cri_km2, file = here::here("results/cri_km2.Rds"))
```


```{r clus_cut, warning = F, message = F, echo = F}
readRDS(cri_km2, file = here::here("results/cri_km2.Rds"))
