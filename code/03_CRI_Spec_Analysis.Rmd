...
ggplot(crispectest) +
  ...
  scale_color_manual(values = c("#E6AB02","NA", "#7570B3"), labels = c("Negative\n(stat. sig.)","Includes Zero\n(n.s.)","Positive\n(stat. sig.)"," ")) +
  #geom_vline(xintercept = 1214, color = "grey", linetype = "dashed") +
  #geom_vline(xintercept = 52, color = "grey", linetype = "dashed") +
  #ylim(-1.8, 1.8) +
  annotate(geom = "text", x = 340, y = 1.45, label = "NEGATIVE\n(stat. sig.)", color = "#E6AB02", fontface = "bold", size = 3) +
  annotate(geom = "text", x = 940, y = 1.45, label = "POSITIVE\n(stat. sig.)", color = "#7570B3", fontface = "bold", size = 3) +
  annotate(geom = "text", x = 640, y = 1.45, label = "INCLUDES ZERO\n(n.s.)", fontface = "bold", color = "grey55", size = 3) +
  ...
  labs(color = "Reported Outcome", x = "Models Ordered by AME", y = "Average Marginal Effect (AME)") +
...
agg_tiff(filename = here::here("results/Fig1.tiff"), width = 3200, height = 1500, res = 432)
ggplot(crispectest) +
  #geom_errorbar(aes(ymin = lb, ymax = ub), color = "thistle1") +
  geom_point(aes(x = count, y = est_ns_scl), color = "grey55", shape = "|", size = 2, show.legend =F) + 
  geom_point(aes(x = count, y = est_sig_scl, color = sig_group), shape = "|", size = 3) +
