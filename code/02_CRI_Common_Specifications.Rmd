...
cri_spec_only03 <- select(cri_spec_only, netmig, west, nonwest, muslim, refugee, main_IV_time, main_IV_effect, Jobs:ChangeFlow, twowayfe:pseudo_pnl, dichotomize)
...
cri_spec_only04 <- select(cri_spec_only, w1985:TR, main_IV_as_control, emigration_ivC:fbXleftright, anynonlin)
...
3) **Measurement, Estimation & Equation** specifications only produces `r length(cri$identical_estmodel[cri$identical_estmodel == 0])`
...
cri_samples <- cri_samples[order(cri_samples$cases, decreasing = T),]
head(cri_samples)
...
cri_samp <- select(cri_samp, identical_samples, w1985:TR)
...
cri_variables <- cri_variables %>%
    ...
    dplyr::summarize(AME_Zm = round(stats::weighted.mean(AME_Z, team_sum),3),
              AME_Zsd = round(sqrt(Hmisc::wtd.var(AME_Z, na.rm = T)),3),
              cases = n(),
              teams = length(unique(u_teamid)),
              Hsupm = round(stats::weighted.mean(Hsup, team_sum),3))
...
cri_var <- select(cri_var, identical_variables, main_IV_as_control, emigration_ivC:fbXleftright)
...
cri_estm  <- select(cri_estm , identical_estmodel, netmig, west, nonwest, muslim, refugee, main_IV_time, main_IV_effect, Jobs:ChangeFlow, twowayfe:pseudo_pnl, dichotomize)
...
write.csv(tbl_ax, file = here::here("results/TblS3.csv"))
```
