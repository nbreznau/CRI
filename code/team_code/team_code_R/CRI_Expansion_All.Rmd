...
team16[1, 1] = as.numeric(round(-0.014/.48,5))
team16[1, 2] = as.numeric(round(-0.028/.48,5))
team16[1, 3] = as.numeric(-0.001) 
team16[1, 4] = "t16m1"
team16[2, 1] = as.numeric(round(0.117/.48,5))
team16[2, 2] = as.numeric(round(-0.291/.48,5))
team16[2, 3] = as.numeric(round(0.487/.48,5))
team16[2, 4] = "t16m2"
...
L2data <- read.dta13(pfunc("L2data_exp2.dta"), nonint.factors = T, generate.factors = TRUE) %>% 
  tbl_df() %>% 
...
issp1985_2016_L2 <- read.dta13(pfunc("issp1985_2016_28.dta"), nonint.factors = T, generate.factors = TRUE) %>% 
...
reslist <- sapply(X=c(form1a, form1b, form1c, form1d, form1e, form1f),
                  FUN=rob_clust_fun, mydat = issp1985_2016_L2, simplify = FALSE)
...
writeFun  = function(DF) {
  write.csv(DF,paste0("OR_",unique(DF$depvar),".csv"), row.names = FALSE)
  return(DF)
}
...
country <- merge(country, aut.id, by.x = "ctry.reg", by.y = "code", all.x = TRUE)
...
issp.rep <- merge(issp.rep, l2, by = c("country", "year"), all.x = TRUE)
save(issp.rep,
...
issp.rep <- issp.rep %>% 
        mutate_at(vars(l2.vars), funs(. - mean(., na.rm = TRUE)))
...
tmp[[paste0("m", imp)]] <- polr(reg_formula,
                              data = issp_imputed$imputations[[imp]], Hess = TRUE)
...
#recode
cri_macro$iso3c <- countrycode(cri_macro$country, "country.name", "iso3c", warn = TRUE)
...
datal2 <- datal2 %>%
  mutate(cntry = iso_country,                                        #just for commenting: 826 probably is GB not UK, might be wrong label
         ...
         gdp = apply(data.frame(gdp_oecd, gdp_wb, gdp_twn), 1, median, na.rm = TRUE),                  #as there are multiple GDP scores that are partially missing: median
         gini = apply(data.frame(ginim_dolt, gini_wb, gini_wid), 1, median, na.rm = TRUE))  %>%        #as there are multiple GINI scores that are partially missing: median
...
oldAge_mean <- mean(data$oldAge, na.rm = TRUE)
unemp_mean <- mean(data$unemployed, na.rm = TRUE)
oldAge_1n <- sum(data$oldAge[data$oldAge == 1], na.rm = TRUE)
oldAge_2n <- (sum(data$oldAge[data$oldAge == 2], na.rm = TRUE))/2
oldAge_3n <- (sum(data$oldAge[data$oldAge == 3], na.rm = TRUE))/3
oldAge_4n <- (sum(data$oldAge[data$oldAge == 4], na.rm = TRUE))/4
unemp_1n <- sum(data$unemployed[data$unemployed == 1], na.rm = TRUE)
unemp_2n <- (sum(data$unemployed[data$unemployed == 2], na.rm = TRUE))/2
unemp_3n <- (sum(data$unemployed[data$unemployed == 3], na.rm = TRUE))/3
unemp_4n <- (sum(data$unemployed[data$unemployed == 4], na.rm = TRUE))/4
