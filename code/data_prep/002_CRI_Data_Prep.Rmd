...
# Team 104 seems to have the scale wrong on by a factor of 10
cri_str <- cri_str %>%
...
```{r pubscount}
# percent who have published in a related area or using a related method
cri_part_desc <- cri_part %>%
    mutate(pubperc = stat6 + topic1 + topic2 + topic3,
           pubpercd = ifelse(pubperc < 5, 0, 1),
           pubpercd = ifelse(is.na(pubperc), 0, pubpercd)) %>%
    select(pubpercd) %>%
    summarise_all(mean)
```
