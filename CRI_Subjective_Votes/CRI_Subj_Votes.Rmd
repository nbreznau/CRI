---
title: "CRI Subjective Model Voting"
output:
  html_document:
    df_print: paged
---
...
The participants voted on model specifications fo the other teams. Due to some errors in the preliminary codes we cannot assign the subjective voting directly to the teams; however, this provided an alternate utility. Based on the variations in model descriptions given to the participants we can still estimate what features of models are seen as the most useful or correct as the data-generating model. We can then look for those features in the actual models. Thus, the models we had participants vote on are theoretical data-generating models that mostly reflect the range of models used in the CRI. 
...
pacman::p_load("readstata13","xlsx","dplyr","tidyr","ggplot2","kableExtra","GLMMadaptive","jtools","sjPlot","sjmisc","sjlabelled","margins","fastDummies","xlsx")
...
The first file "Research Design Votes.xlsx" is the file we used to generate the model descriptions in the Unipark survey. 

The second file "votes.dta" was generated in the CRI work-up. See CRI_Spec_Analysis.Rmd.

```{r merge}
model_desc <- read.xlsx(wdir("Research Design Votes.xlsx"), sheetName = "RD Paragraph Output", header = T, stringsAsFactors = F)
...
votes <- read.dta13(wdir("votes.dta"))
model_desc <- left_join(model_desc,votes, by = "i")

model_desc <- select(model_desc, i, vote_id, d, everything())
...
cor <- select(model_desc, model_x, level, mator, model_spec, year, dv_dichot, dv_linear, dv_extra, dv_cat, rich13, rich17, eeurope)
...
cord1 <- cord[,1:25]
cord2 <- cord[,26:51]
# write.xlsx(cord, wdir("cord.xlsx"))
...
png(file = "Fig1_meta.png", width = 750)
ggplot(popdf, aes(x=xaxis, y=Avg_ScoreA, label = Feature)) +
  ...
  coord_flip() + labs(y = "Average Relative Support, (OLS = 0, negative values trimmed)", x = "") +
...
# Variables to use:
model_desca <- model_desc
# only two cases of country-year, year, combine with country-year, country
model_desca$level = ifelse(model_desca$level=="country-yearyear", "country-yearcountry", model_desca$level)
# same with level = None, combine with year
model_desca$level = ifelse(model_desca$level=="None", "year", model_desca$level)

m1 <- lm(vote_subj ~ model_form , data= model_desca) # alone this explains nothing, also adds nothing to later models
m2 <- lm(vote_subj ~ model_x, data=model_desca) 
m3 <- lm(vote_subj ~ model_x + level, data=model_desca)
m4 <- lm(vote_subj ~ model_x + level + mator, data=model_desca)
m5 <- lm(vote_subj ~ model_x + level + mator + model_spec, data=model_desca)
m6 <- lm(vote_subj ~ model_x + level + mator + model_spec + year, data=model_desca)
m7 <- lm(vote_subj ~ model_x + level + mator + model_spec + year + dv_dichot, data=model_desca)

m2 <- lm(vote_subj ~ model_x + level + mator + model_spec + year + dv_dichot + dv_linear + dv_extra + dv_cat + rich13 + rich17 + eeurope, data=model_desca)
m2 <- lm(vote_subj ~ model_x + level + mator + model_spec + year + dv_dichot + dv_linear + dv_extra + dv_cat + rich13 + rich17 + eeurope, data=model_desca)
m2 <- lm(vote_subj ~ model_x + level + mator + model_spec + year + dv_dichot + dv_linear + dv_extra + dv_cat + rich13 + rich17 + eeurope, data=model_desca)
m2 <- lm(vote_subj ~ model_x + level + mator + model_spec + year + dv_dichot + dv_linear + dv_extra + dv_cat + rich13 + rich17 + eeurope, data=model_desca)


tab_model(m2, m3,m4,m5,m6,m7,p.style = "stars", show.ci = F)
...
Brady & Finnigan did not use clustered standard errors in the two-way FE models at the country-level. Therefore, coefficient significance tests use thousands of cases when there are only 13 countries. Therefore, to truly test the CRI hypothesis every study must cluster the standardd errors for all country-level independent variable coefficients. Otherwise the estimates are untrustworthy.
...
We import the master data, make a column for each model specification, and then create a total score for model. 


```{r output1, message = F, warning = F}
...
popdf <- select(popdf, Feature, score)
...
d2 <- data.frame(matrix(nrow = 1312, ncol = 33))
...
popdf_out <- popdf_out %>%
  mutate(`All 5-Waves` = max(as.numeric(`All 5-Waves`), na.rm=T),
         ...
         `Includes Eastern Europe` = max(as.numeric(`Includes Eastern Europe`), na.rm = T),
...
```{r output2, message = F, warning = F}

crispec <- read.xlsx(wdir("CRI Model Specifications and Margins.xlsx"), sheetName = "Data")
...
popdf_out <- select(popdf_out, id, score_waves, score_cases, score_levels, score_mlm, score_mator, score_weight)
...
popdf_out <- popdf_out %>%
  rowwise() %>%
  mutate(total_score = mean(c(score_waves, score_cases, score_levels, score_mlm, score_mator, score_weight), na.rm = T))

save(popdf_out, file = wdir("popdf_out.Rdata"))
