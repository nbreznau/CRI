...
pacman::p_load("dplyr","knitr","kableExtra","reshape","ggplot2","ragg","summarytools")
...
codebook <- select(codebook, DV, main_IV_type, main_IV_measurement, main_IV_time, num_countries, main_IV_as_control, software, mator, indepv) %>%
...
codebook1 <- gather(codebook, "var", "value") %>%
  count(var, value) %>%
  group_by(var) %>%             
...
codebook1 <- codebook1 %>%
  split(.$var) %>% 
  map(~add_row(., var=unique(.$var), value=NA, n=1000, prop=NA)) %>%
  bind_rows()
...
codebook1 <- codebook1 %>%
  split(.$var) %>% 
  map(~add_row(., var=unique(.$var), value=NA, n=0, prop="100.00%")) %>%
  bind_rows()
