...
crispectest <- crispectest %>%
  mutate(DV_Jobs = ifelse(is.na(Jobs), NA, 1), # Measurement
         DV_Unemp = ifelse(is.na(Unemp), NA, 1),
         DV_IncDiff = ifelse(is.na(IncDiff), NA, 1),
         DV_OldAge = ifelse(is.na(OldAge), NA, 1),
         DV_House = ifelse(is.na(House), NA, 1),
         DV_Health = ifelse(is.na(Health), NA, 1),
         DV_Scale = ifelse(is.na(Scale), NA, 1), 
         Test_Stock = ifelse(iv_type == "Stock", 1, NA),
         Test_Flow = ifelse(iv_type == "Flow", 1, NA),
         Test_ChangeFlow = ifelse(iv_type == "ChangeFlow", 1, NA),
         W_Other = ifelse(SURVEY_WAVES == "Other", 1, NA), # Sample
         W_1996 = ifelse(SURVEY_WAVES == "w1996", 1, NA),
         W_2006 = ifelse(SURVEY_WAVES == "w2006", 1, NA),
         W_2016 = ifelse(SURVEY_WAVES == "w2016", 1, NA),
         W_96_06 = ifelse(SURVEY_WAVES == "1996 & 2006", 1, NA),
         W_90_06_16 = ifelse(SURVEY_WAVES == "1996, 2006 & 2016", 1, NA),
         W_06_16 = ifelse(SURVEY_WAVES == "2006 & 2016", 1, NA),
         C_13 = ifelse(COUNTRIES == "13 Rich Democracies Only", 1, NA),
         C_Other = ifelse(COUNTRIES == "Other", 1, NA),
         C_All = ifelse(COUNTRIES == "All Available (~26)", 1, NA),
         C_CEE = ifelse(COUNTRIES == "Includes Eastern/CE Europe", 1, NA),
         )
...
g7 <- ggplot(crispectest) +
  geom_tile(aes(x = count, y = 7, fill = Jobs))
```
