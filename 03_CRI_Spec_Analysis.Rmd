...
g1a <- ggplot(crispectest) +
  ...
  scale_color_manual(values = c("chartreuse4","NA", "firebrick"), labels = c("Negative","Not sig.","Positive"," ")) +
  ylim(-0.21, 0.21) +
  labs(color = "Effect at 95%CI", x = "Standardized Effect Size", y = " \n \n \n ") +
  theme_classic() +
  guides(color = guide_legend(override.aes = list(size=7, color=c("chartreuse4","grey55", "firebrick","NA")))) +
  theme(
    legend.key.size = unit(2,"line"),
...
```{r fig1_col1}
agg_png(filename = "results/Fig1_Col1.png", height = 800, width = 400, res = 72)
ggarrange(g1a, g01, g02, g03, g04, nrow = 5, ncol = 1, heights = c(0.6,.75,0.5,1.75,1.2))
dev.off()
