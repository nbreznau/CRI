...
```{r fig1_main}
agg_png(filename = "results/Fig1.png", width = 800, height = 500, res = 144)
ggplot(crispectest) +
  ...
  #ylim(-1.8, 1.8) +
  annotate(geom = "text", x = 150, y = 0.4, label = "NEGATIVE MARGIN, 95%CI\n(direction with hypothesis)", color = "chartreuse4", hjust = 0, size = 3) +
  annotate(geom = "text", x = 750, y = -0.4, label = "POSITIVE MARGIN, 95%CI\n(direction against hypothesis)", color = "firebrick", hjust = 0, size = 3) +
  ...
  annotate(geom = "segment", x = -35, xend = -25, y = 1.1, yend = 1.1, color = "white", size = 2) +
  labs(color = "Effect at 95%CI", x = "Model", y = "Average Marginal Effect\nxy-Standarized") +
...
#### Fig 1_supplementary. Main Visual Results
...
agg_png(filename = "results/Fig2team_new2.png", res = 144, height = 900, width = 1200)
ggarrange(g5a_new,g5b_new,ncol = 1, heights = c(1,1.2))
dev.off()

knitr::include_graphics("results/Fig2team_new2.png")
```


```{r saveout}
write.csv(crispectest, file = "results/crispectest.csv")
```
