...
pacman::p_load("devtools","ggplot2","dplyr","readr","ExPanDaR","plotscale","lattice","tidyr","readxl","mlogit","metaviz","jtools","sjPlot","sjmisc","sjlabelled","knitr","boot","ragg","kableExtra", "arsenal")
...
load(file = wdir("Env1.Rdata"))

# put team_id back into dataset (this should be done in an early command file, change at some point)

idm <- select(cri, u_teamid, id)
crispectest <- left_join(crispectest, idm, by = "id")
...
# mistake in countries (will be fixed next time code is run from scratch)

cri$countries <- ifelse(cri$countries == 0, 1, cri$countries)
...
# select variables
cri_desc  <- select(cri, AME, AME_Z, p, Hresult, inv_weight, countrynum, total_score, stata, r, other, ols, logit, otherE, main_IV_type)

t1 <- tableby(main_IV_type ~ ., data = cri_desc)

my_labels <- list(
  AME = "Avg. Marginal Effect",
  AME_Z = "Avg. Marginal Effect, Standardized",
  p = "p-value of AME",
  Hresult = "Subjective Conclusion",
  inv_weight = "Models per Team",
  countrynum = "Number of Countries in Model",
  total_score = "Subjective Vote: Quality of Model",
  stata = "Software: Stata",
  r = "Software: R",
  other = "Sofware: other",
  ols = "Estimator: OLS",
  logit = "Estimator: Logit",
  otherE = "Estimator: Other"
)

summary(t1, labelTranslations = my_labels)
```
