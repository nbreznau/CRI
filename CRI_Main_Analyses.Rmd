...
pacman::p_load("devtools","ggplot2","dplyr","readr","ExPanDaR","plotscale","lattice","tidyr","readxl","mlogit","metaviz","jtools","sjPlot","sjmisc","sjlabelled","knitr","boot","ragg","kableExtra")
...
cri <- cri %>%
  mutate(other = ifelse(stata == 0 & r == 0, 1, 0),
         otherE = ifelse(ols == 0 & logit == 0, 1, 0), 
         ...
         countrynum = ifelse(countries < 10, "< 10", ifelse(countries > 9 & countries < 17, "10-16", ifelse(countries > 16 & countries < 26, "17-25", ifelse(countries > 25, "> 25", NA)))))

cri <- cri %>%
  mutate(jobs = ifelse(Jobs == 0, 0, 1),
         unemp = ifelse(Unemp == 0, 0, 1),
         incdiff = ifelse(IncDiff == 0, 0, 1),
         oldage = ifelse(OldAge == 0, 0, 1),
         house = ifelse(House == 0, 0, 1),
         health = ifelse(Health == 0, 0, 1),
         scale = ifelse(Scale == 0, 0, 1),
         wave_1985 = ifelse(w1985 == 0, 0, 1),
         wave_1990 = ifelse(w1990 == 0, 0, 1),
         wave_1996 = ifelse(w1996 == 0, 0, 1),
         wave_2006 = ifelse(w2006 == 0, 0, 1),
         wave_2016 = ifelse(w2016 == 0, 0, 1),
         Hno = ifelse(Hresult == "No test", 1, 0),
         iv_stock = ifelse(main_IV_type == "Stock", 1, 0),
         iv_flow = ifelse(main_IV_type == "Flow", 1, 0),
         iv_change_flow = ifelse(main_IV_type == "Change in Flow", 1, 0)
  )


sum1 <- summarise_at(cri, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(mean), na.rm = T)

sum1.1 <- summarise_at(cri, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(sd), na.rm = T)

# need to fix to get accurate N counts at some point
sum2 <- summarise_at(cri, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(n()))

sum3 <- summarise_at(cri, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(min), na.rm = T)

sum4 <- summarise_at(cri, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(max), na.rm = T)

sum1[2,] <- c("AME","AME, beta","p-value","Support Hypothesis", "Reject Hypothesis", "Not testable Hypothesis", "DV: Jobs", "DV: Unemp", "DV: Income Diffs","DV: Old Age", "Dv: Housing", "DV: Health care", "DV: Constructed Scale", "ISSP Wave: 1985", "ISSP Wave: 1990", "ISSP Wave: 1996", "ISSP Wave: 2006", "ISSP Wave: 2016", "# of countries used", "Dichotomized DV", "Test IV: Stock of Immigrants", "Test IV: Flow of Immigrants", "Test IV: Change in Flow","Both Test IVs included","Two-Way FEs (year & country dummies","Any form of SE clutering","Number of Models (by team)","Team size","Software: Stata","software: R","Software: SPSS","Software: Mplus","software: MLwin","Subjective model score","Statistics skills (by team)","Belief that Hypothesis is true (by team)","Interst in the research topic (by team)")
...
```{r desc_team}
cri_team <- select(cri, u_teamid, AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic)

cri_team <- aggregate(cri_team, FUN = mean, na.rm = T, by = list(cri_team$u_teamid))

sum1 <- summarise_at(cri_team, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(mean), na.rm = T)

sum1.1 <- summarise_at(cri_team, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(sd), na.rm = T)

# need to fix to get accurate N counts at some point
sum2 <- summarise_at(cri_team, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(n()))

sum3 <- summarise_at(cri_team, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(min), na.rm = T)

sum4 <- summarise_at(cri_team, vars(AME, AME_Z, p, Hsup, Hrej, Hno, jobs:wave_2016, num_countries, dichotomize,  iv_stock, iv_flow, iv_change_flow, main_IV_as_control, twowayfe, cluster_any, inv_weight, team_size, stata, r, spss, mplus, mlwin, total_score, stat, belief, topic), funs(max), na.rm = T)

sum1[2,] <- c("AME (team average)","AME, beta (team average)","p-value (team average)","Support Hypothesis", "Reject Hypothesis", "Not testable Hypothesis", "DV: Jobs (team average)", "DV: Unemp (team average)", "DV: Income Diffs (team average)","DV: Old Age (team average)", "Dv: Housing (team average)", "DV: Health care (team average)", "DV: Constructed Scale (team average)", "ISSP Wave: 1985 (team average)", "ISSP Wave: 1990 (team average)", "ISSP Wave: 1996 (team average)", "ISSP Wave: 2006 (team average)", "ISSP Wave: 2016 (team average)", "# of countries used (team average)", "Dichotomized DV (team average)", "Test IV: Stock of Immigrants (team average)", "Test IV: Flow of Immigrants (team average)", "Test IV: Change in Flow (team average)","Both Test IVs included (team average)","Two-Way FEs (year & country dummies (team average)","Any form of SE clutering (team average)","Number of Models ","Team size","Software: Stata","software: R","Software: SPSS","Software: Mplus","software: MLwin","Subjective model score","Statistics skills","Belief that Hypothesis is true","Interst in the research topic")
